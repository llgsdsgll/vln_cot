...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/setup_thor_navgen_env.sh [options]
...
Notes:
  1. This script targets the NavGen generation pipeline, not model training.
  2. It keeps the env name as lhvln-cu128 by default so existing batch scripts
     in this repo can run without modification.
  3. It assumes thor is a regular Linux machine similar to the current
     verified setup, with CUDA 12.8-compatible PyTorch desired by default.
EOF
}
...
RUNTIME_REQUIREMENTS="$REPO_ROOT/requirements_navgen_runtime.txt"
...
echo "[4/6] Installing PyTorch wheels"
...
echo "[5/6] Installing NavGen runtime Python dependencies"
"$CONDA_EXE" run -n "$ENV_NAME" python -m pip install -r "$RUNTIME_REQUIREMENTS"

echo "[6/6] Writing activation hints"
ACTIVATE_HINT="$REPO_ROOT/scripts/activate_${ENV_NAME}.sh"
